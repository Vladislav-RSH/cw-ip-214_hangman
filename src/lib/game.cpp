...
void Game_shell::run()
{
    // Main game element - ideally refactored into a seperate function at a
    // later date
    _attempts = 0;
    Game_data session;
    if (!_multiplayer) {
    ...
    while (_attempts < GAMELENGTH - 1) {
        ...
        if (session.endGame()) {
            cout << "\nCongratulations " << _guess._name << "- You won!!"
                 << endl;
            _guess._score++;
            break;
        }
...
{
    // For single player games, payer gains a point for guessing correctly,
    // loses one for incorrectly guessing
