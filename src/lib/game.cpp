...
// Globals
#define FILELENGTH \
  1386  // IF words.txt IS EDITTED, THIS VALUE MUST BE UPDATED!!
#define GAMELENGTH \
  9  // Can be changed to 11 if the 2 elements in stage are uncommented

// The gallows - Looks scary but just draws the acutal hangman in the terminal
// interface
...
void Game_shell::run() {
  // Main game element - ideally refactored into a seperate function at a later
  // date
  _attempts = 0;
  Game_data session;
  if (!_multiplayer) {
...
void Game_shell::playerScore() {
  // For single player games, payer gains a point for guessing correctly, loses
  // one for incorrectly guessing
...
// Constructor
Game_data::Game_data() { _partialSol = ""; }

// Generates the word to guess, either via user input or random selecting a word
// from words
...
void Game_data::addWord(Player guess, Player word) {
  HideStdinKeystrokes();
  cout << "\n"
       << word._name << " - Please provide a word for " << guess._name
       << " to guess:";
  cin >> _word;
  ShowStdinKeystrokes();
  // cout << word;     //DEBUG
...
// Displays current solution, incorrect guess and how badly the man has been
// hanged so far
...
bool Game_data::playerGuess() {
  ...
  cin >> _guess;
  bool ans = false;
  // Iterate through partial solution and incorrect guesses to check for human
  // failure
  for (unsigned int i = 0; i < _incorrect.size(); i++) {
